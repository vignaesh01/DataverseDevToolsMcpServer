...
- MCP support enabled in Copilot Chat (Preview/Experimental in some builds)
...
- Add a local MCP server in Copilot Chat settings and point it to the executable/args below, or
- If your VS build supports reading a JSON config, reuse the same `mcp.json` you created for VS Code.

Executable options:
- From global tool (Recommended): `dataversedevtoolsmcpserver --environmentUrl https://yourorg.crm.dynamics.com`
- From source: `dotnet run --project C:/Projects/DataverseDevToolsMcpServer/DataverseDevToolsMcpServer/DataverseDevToolsMcpServer.csproj --environmentUrl https://yourorg.crm.dynamics.com`
